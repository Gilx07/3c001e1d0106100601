(()=>{
  ...
  function parsePost(html){
    ...
    return {pageTitle,description,date:meta[0]?.textContent?.trim()||'',category:meta[1]?.textContent?.trim()||'',body:article?.innerHTML.trim()||''};
  }

  function setMeta(doc,selector,attribute,value){
  ...
  function serializePost(){
    ...
    const fullTitle=`${title} — Dolenthis`;
    const canonical=`https://dolenthis.my.id/${selectedPath}`;

    setMeta(parsed,'meta[name="description"]','content',description);
    setMeta(parsed,'meta[property="og:title"]','content',fullTitle);
    setMeta(parsed,'meta[property="og:description"]','content',description);
    setMeta(parsed,'meta[property="og:url"]','content',canonical);
    setMeta(parsed,'meta[name="twitter:title"]','content',fullTitle);
    setMeta(parsed,'meta[name="twitter:description"]','content',description);
    const canonicalEl=parsed.querySelector('link[rel="canonical"]');
    ...
    if(!meta){
      meta=parsed.createElement('div');meta.className='article-meta';
      const rule=parsed.querySelector('.page-rule');rule?.parentNode?.insertBefore(meta,rule);
    }
    meta.replaceChildren();
    const dateSpan=parsed.createElement('span');dateSpan.textContent=dateInput.value.trim();
    const categorySpan=parsed.createElement('span');categorySpan.textContent=categoryInput.value.trim();
  ...
  async function syncBlogListing(){
    ...
    const doc=new DOMParser().parseFromString(decodeBase64(indexFile.content||''),'text/html');
    ...
    const title=card.querySelector('h2'),description=card.querySelector('.post-copy p'),date=card.querySelector('.post-date');
    ...
    return api(`/repos/${OWNER}/${REPO}/contents/blog.html`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:`content: sync blog listing for ${selectedPath}`,content:encodeBase64(updated),sha:indexFile.sha,branch:BRANCH})});
  }

  async function connect(){
    const entered=tokenInput.value.trim();if(entered)token=entered;
  ...
  async function loadPosts(){
    refreshBtn.disabled=true;postList.replaceChildren();
    const loading=document.createElement('div');loading.className='account';loading.textContent='Memuat daftar post...';postList.appendChild(loading);
    try{
      ...
      if(!posts.length){const empty=document.createElement('div');empty.className='account';empty.textContent='Belum ada file blog-*.html.';postList.appendChild(empty);return;}
      for(const item of posts){
        const button=document.createElement('button');button.type='button';button.className='post-item';button.dataset.path=item.path;
        const label=document.createElement('strong');label.textContent=item.name.replace(/^blog-|\.html$/gi,'').replace(/-/g,' ');
        const path=document.createElement('span');path.textContent=item.path;button.append(label,path);button.addEventListener('click',()=>loadPost(item.path,button));postList.appendChild(button);
      }
    }catch(error){postList.replaceChildren();const failure=document.createElement('div');failure.className='account';failure.textContent=`Gagal memuat post: ${error.message}`;postList.appendChild(failure);}
    finally{refreshBtn.disabled=false;}
  }

  async function loadPost(path,button){
    setStatus(editorStatus,'');document.querySelectorAll('.post-item').forEach(item=>item.classList.toggle('active',item===button));button.disabled=true;
    try{
      const file=await api(`/repos/${OWNER}/${REPO}/contents/${encodePath(path)}?ref=${encodeURIComponent(BRANCH)}`);
      const html=decodeBase64(file.content||''),parsed=parsePost(html);
      selectedPath=path;selectedSha=file.sha;originalHtml=html;currentPath.textContent=path;
      titleInput.value=parsed.pageTitle;descriptionInput.value=parsed.description;dateInput.value=parsed.date;categoryInput.value=parsed.category;bodyInput.value=parsed.body;commitInput.value=`content: update ${path}`;
      emptyEditor.classList.add('hidden');editor.classList.remove('hidden');
    }catch(error){setStatus(editorStatus,`Gagal membuka post: ${error.message}`,'error');}
    finally{button.disabled=false;}
  }

  async function savePost(){
    if(!selectedPath||!selectedSha){setStatus(editorStatus,'Pilih post terlebih dahulu.','error');return;}
    saveBtn.disabled=true;setStatus(editorStatus,'Memeriksa versi terbaru...');
    try{
      ...
      const html=serializePost(),message=commitInput.value.trim()||`content: update ${selectedPath}`;
      setStatus(editorStatus,'Menyimpan artikel...');
      const result=await api(`/repos/${OWNER}/${REPO}/contents/${encodePath(selectedPath)}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({message,content:encodeBase64(html),sha:selectedSha,branch:BRANCH})});
      selectedSha=result.content.sha;originalHtml=html;setStatus(editorStatus,'Artikel tersimpan. Menyinkronkan daftar Blog...');
      ...
      if(error.status===403){detail+=' · HTTP 403.';if(error.acceptedPermissions)detail+=` GitHub meminta: ${error.acceptedPermissions}.`;detail+=' Disconnect, masukkan ulang token terbaru, dan pastikan Fine-grained PAT memakai Resource owner Gilx07 serta Contents: Read and write.';}
      if(error.requestId)detail+=` Request ID: ${error.requestId}`;setStatus(editorStatus,detail,'error');
